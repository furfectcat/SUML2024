...
def fetch_exchange_rates(currency, start_date, end_date):
    ...
    while current_start_date < end_date:
        ...
        current_end_date = min(current_start_date.replace(year=current_start_date.year + 1) - timedelta(days=1), end_date)
        ...
        if response.status_code == 200:
            ...
            print(f"Fetched data for {currency} from {current_start_date.strftime('%Y-%m-%d')} to {current_end_date.strftime('%Y-%m-%d')}")
        else:
            print(f"Error fetching data for {currency} from {current_start_date.strftime('%Y-%m-%d')} to {current_end_date.strftime('%Y-%m-%d')}: {response.status_code}")
...
def save_to_csv(currency, data):
    filename = f"{currency.upper()}_exchange_rates.csv"
